...
export interface Account { id: string; name: string; type: 'cash' | 'bank' | 'wallet' | 'savings' | 'debit' | 'credit'; openingBalance: number; creditLimit?: number; color: string; createdAt: string; }
...
export interface Recurring { id: string; name: string; accountId?: string; categoryId?: string; kind: TransactionKind; amount: number; cadence: 'weekly' | 'monthly' | 'yearly'; nextDate: string; startDate?: string; endDate?: string; active: boolean; }
