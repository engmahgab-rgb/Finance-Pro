...
export interface Account { id: string; name: string; type: AccountType; openingBalance: number; creditLimit?: number; statementDay?: number; paymentDueDay?: number; color: string; createdAt: string; archived?: boolean; }
export interface Category { id: string; name: string; icon: string; color: string; kind: 'income' | 'expense'; }
export interface Transaction { id: string; accountId: string; destinationAccountId?: string; categoryId?: string; kind: TransactionKind; amount: number; date: string; note?: string; tags: string[]; createdAt: string; status?: TransactionStatus; recurringId?: string; debtId?: string; updatedAt?: string; }
export interface Budget { id: string; categoryId: string; month: string; limit: number; }
export interface Recurring { id: string; name: string; accountId?: string; categoryId?: string; kind: Extract<TransactionKind, 'income' | 'expense' | 'credit_purchase'>; amount: number; cadence: 'weekly' | 'monthly' | 'yearly'; nextDate: string; startDate?: string; endDate?: string; totalInstallments?: number; paidInstallments?: number; active: boolean; skippedDates?: string[]; }
...
export interface AppSettings { id: 'app'; currency: string; darkMode: boolean; displayName?: string; language?: 'en' | 'ar'; appearance?: 'light' | 'dark' | 'system'; googleClientId?: string; }
