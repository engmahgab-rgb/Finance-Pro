...
export interface Recurring { id: string; name: string; accountId?: string; categoryId?: string; kind: Extract<TransactionKind, 'income' | 'expense' | 'credit_purchase'>; amount: number; cadence: RecurringCadence; monthlyReserve?: number; nextDate: string; startDate?: string; endDate?: string; totalInstallments?: number; paidInstallments?: number; active: boolean; skippedDates?: string[]; }
