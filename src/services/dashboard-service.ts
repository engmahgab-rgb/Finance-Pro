...
export function calculateDashboardMetrics(accounts: Account[], transactions: Transaction[], categories: Category[], periodTransactions = transactions): DashboardMetrics {
  const summary = summarizeLedger(accounts, periodTransactions);
  ...
  for (const transaction of periodTransactions) if (transaction.status !== 'void' && (transaction.kind === 'expense' || transaction.kind === 'credit_purchase') && transaction.categoryId) spending.set(transaction.categoryId, (spending.get(transaction.categoryId) ?? 0) + transaction.amount);
